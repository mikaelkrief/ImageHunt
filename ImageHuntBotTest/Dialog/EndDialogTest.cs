...
{
    public class EndDialogTest : BaseTest
    {
      private IEndDialog _target;
      private ILogger _logger;
        private IActionWebService _actionWebService;

        public EndDialogTest()
      {
        _testContainerBuilder.RegisterType<EndDialog>();
          _actionWebService = A.Fake<IActionWebService>();
          _testContainerBuilder.RegisterInstance(_actionWebService).As<IActionWebService>();
        _logger = A.Fake<ILogger<EndDialog>>();
        _testContainerBuilder.RegisterInstance(_logger).As<ILogger<EndDialog>>();

        _container = _testContainerBuilder.Build();
        _target = _container.Resolve<EndDialog>();
      }
