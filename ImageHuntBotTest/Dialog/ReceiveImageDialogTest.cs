...
namespace ImageHuntBotTest
{
    public class ReceiveImageDialogTest : BaseTest
    {
        ...
        public ReceiveImageDialogTest()
        {
            ...
            _logger = A.Fake<ILogger<ReceiveImageDialog>>();
            _testContainerBuilder.RegisterInstance(_logger).As<ILogger<ReceiveImageDialog>>();
        ...
        public async Task Begin_GameNotStarted()
        {
            // Arrange
            var turnContext = A.Fake<ITurnContext>();
            var imageHuntState = new ImageHuntState() { GameId = 15, TeamId = 16, CurrentLatitude = 15.2, CurrentLongitude = 56, Status = Status.Initialized };
            A.CallTo(() => turnContext.GetConversationState<ImageHuntState>()).Returns(imageHuntState);
            var photoSize1 = new PhotoSize() { FileSize = 15, FileId = "15" };
            var photoSize2 = new PhotoSize() { FileSize = 1195247, FileId = "AgADBAADOawxG-RQqVO-4ni8OVZOPOnykBkABDQFk1xY-YUAAR0SAgABAg" };
            var activity = new Activity()
        ...
        [Fact]
        public async Task ReceiveQRCode()
        {
            // Arrange
            var turnContext = A.Fake<ITurnContext>();
            var imageHuntState = new ImageHuntState() { TeamId = 16, CurrentLatitude = 15.2, CurrentLongitude = 56 };
            A.CallTo(() => turnContext.GetConversationState<ImageHuntState>()).Returns(imageHuntState);
            var photoSize = new PhotoSize() { FileSize = 1195247, FileId = "AgADBAADOawxG-RQqVO-4ni8OVZOPOnykBkABDQFk1xY-YUAAR0SAgABAg" };
