...
namespace ImageHuntCore.Model
{
    public class Team : DbObject
    {
        ...
        [NotMapped] public IEnumerable<Player> Players => TeamPlayers.Select(tp => tp.Player);

        [NotMapped] public IEnumerable<Passcode> Passcodes => TeamPasscodes.Select(tp => tp.Passcode);
