...
namespace ImageHuntCore.Computation
{
    public static class GeographyComputation
    {
        ...
        public static (double Latitude, double Longitude) CenterOfGeoPoints(
            IEnumerable<(double Latitude, double Longitude)> points)
        {
            var radPoints = new List<(double, double)>();
            // convert points in radian
            foreach (var point in points) radPoints.Add(DegToRad(point));
            ...
            var average =
                cartesianPoints.Aggregate((A, B) => (A.Item1 + B.Item1, A.Item2 + B.Item2, A.Item3 + B.Item3));
        ...
        public static double Distance(this (double Latitude, double Longitude) point1,
            (double Latitude, double Longitude) point2)
        {
            ...
            return Math.Acos(Math.Sin(radPoint1.Item1) * Math.Sin(radPoint2.Item1) + Math.Cos(radPoint1.Latitude) *
                             Math.Cos(radPoint2.Latitude) * Math.Cos(radPoint2.Longitude - radPoint1.Longitude)) *
                   EarthRayon;
        }
        ...
        public static double Distance(double lat1, double lng1, double lat2, double lng2)
        {
            ...
            return Math.Acos(Math.Sin(radPoint1.Latitude) * Math.Sin(radPoint2.Latitude) + Math.Cos(radPoint1.Item1) *
                             Math.Cos(radPoint2.Item1) * Math.Cos(radPoint2.Longitude - radPoint1.Longitude)) *
                   EarthRayon;
        }
    }
}