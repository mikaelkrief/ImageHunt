...
namespace ImageHuntTelegramBot
{
    ...
    public class DictionaryStorage : IStorage
    {
        ...
        public Task<IEnumerable<IEnumerable<KeyValuePair<string, object>>>> ReadAll()
        {
            var storesItems = new List<List<KeyValuePair<string, object>>>();
            return Task.FromResult<IEnumerable<IEnumerable<KeyValuePair<string, object>>>>(storesItems);
        }
            ...
            lock (_syncroot)
            {
                foreach (var change in changes)
                {
                    ...
                    if (_memory.TryGetValue(change.Key, out var oldState))
                    {
                        ...
                        {
                            oldStateETag = Extensions.Value<string>(eTagToken);
                        }
                    }
    ...
    {
        public MemoryStorage() : base((Dictionary<string, JObject>)null)
        {
        }
    }
}