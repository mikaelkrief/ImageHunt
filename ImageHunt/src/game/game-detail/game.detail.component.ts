...
export class GameDetailComponent implements OnInit {
  ...
  getGame(gameId: number) {
    forkJoin([
      ...
      .subscribe(responses => {
          this.game = ((responses[0]) as Game);
          this.currentZoom = this.game.mapZoom;
          this.nodes = ((responses[1]) as NodeResponse[]);
          this.buildRelations();
  ...
  centerMap(gameId: number) {
    this._gameService.centerMap(gameId).subscribe(null,
      null,
      () => {
        this._alertService.sendAlert("Centrage de la map effectué", "success", 1000);
        this._gameService.setZoom(gameId,
            this.currentZoom == undefined || this.currentZoom === 0 ? this.game.mapZoom : this.currentZoom)
    ...
    if (nodeClicked.numberClicked === 1) {
      ...
      if (nodeClicked.node.nodeType === "QuestionNode") {
        ...
        this._alertService.sendAlert(
          `Editez les relations des noeuds Question dans le module d'édition des réponses aux questions`,
          "danger",
          5000);
        return;
      }
      if ((nodeClicked.node.nodeType === "FirstNode" ||
        ...
        this.mapComponent.resetNodeClick();
        this._alertService.sendAlert(`Le noeud ${nodeClicked.node.name} ne peut pas accepter d'avantage d'enfants`,
          "danger",
          5000);
  ...
  newRelation(nodeRelation: NodeRelation) {
    const parentNode = this.game.nodes.find(n => n.id === nodeRelation.nodeId);
    const childNode = this.game.nodes.find(n => n.id === nodeRelation.childNodeId);
  ...
  saveEditedRelations(editedRelations: EditedRelation[]) {
    for (let relation of editedRelations) {
