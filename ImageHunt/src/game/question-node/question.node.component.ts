...
export class QuestionNodeComponent implements OnInit {
  ...
  associateAnswerToNode() {
    if (this.selectedAnswer != null && this.selectedTargetNode != null) {
      this.answersNodeRelations.push({
        ...
        answerName: this.selectedAnswer.response,
        nodeId: this.selectedTargetNode.id,
        nodeName: this.selectedTargetNode.name
      });
      const orgNode = this.nodes.find(n => n.id == this.selectedNode.nodeId);
  ...
  dissociateAnswerToNode() {
    if (this.selectedAnswer != null && this.selectedTargetNode != null) {
      const index = this.answersNodeRelations.findIndex(l => l.answerId === this.selectedAnswer.id &&
        l.nodeId === this.selectedTargetNode.id);
  ...
  saveRelations() {
    const relationAnswers =
      this.answersNodeRelations.map(
        a => new QuestionNodeAnswerRelation(this.selectedNode.nodeId, a.nodeId, a.answerId));
    this._gameService.addRelationAnswers(relationAnswers)
      .subscribe(res => {
        this._alertService.sendAlert(`Les relations du noeud ${this.selectedNode.name} ont été sauvegardées`,
          "success",
          5000);
