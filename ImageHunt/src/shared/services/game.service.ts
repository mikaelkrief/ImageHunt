...
export class GameService {
  ...
  addRelation(orgNodeId: number, destNodeId: number, answerId: number) {
    ...
    return this.http.put("api/Node/AddRelationToNode",
      { nodeId: orgNodeId, childrenId: destNodeId, answerId: answerId });
  ...
  getPictureSubmissionsToValidateForGame(gameId: number,
    pageIndex: number,
    pageSize: number,
    nbProbableNode: number,
    teamId?: number) {
    if (teamId)
      return this.http.get<GameAction[]>(
        `api/Action/GameActionsToValidate?gameId=${gameId}&teamId=${teamId}&pageIndex=${pageIndex}&pageSize=${pageSize
        }&nbPotential=${nbProbableNode}&includeAction=Picture`);
    return this.http.get<GameAction[]>(
      `api/Action/GameActionsToValidate?gameId=${gameId}&pageIndex=${pageIndex}&pageSize=${pageSize}&nbPotential=${
      nbProbableNode}&includeAction=Picture`);
  }

  getHiddenActionToValidateForGame(gameId: number,
    pageIndex: number,
    pageSize: number,
    nbProbableNode: number,
    teamId?: number) {
    if (teamId)
      return this.http.get<GameAction[]>(
        `api/Action/GameActionsToValidate?gameId=${gameId}&teamId=${teamId}&pageIndex=${pageIndex}&pageSize=${pageSize
        }&nbPotential=${nbProbableNode}&includeAction=HiddenNode`);
    return this.http.get<GameAction[]>(
      `api/Action/GameActionsToValidate?gameId=${gameId}&pageIndex=${pageIndex}&pageSize=${pageSize}&nbPotential=${
      nbProbableNode}&includeAction=HiddenNode`);
  }
