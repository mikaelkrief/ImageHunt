...
export class UserRoleComponent implements OnInit {
  ...
  saveUser(user) {
    this._userService.saveUser(user)
      .subscribe(res => {
          this._alertService.sendAlert(`L'utilisateur ${user.userName} a bien été sauvegardé`, "success", 3000);
        },
        error => {
          this._alertService.sendAlert(`Erreur lors de la sauvegarde de l'utilisateur ${user.userName}`,
            "danger",
            10000);
        });
  }
