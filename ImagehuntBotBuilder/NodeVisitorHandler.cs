...
namespace ImageHuntBotBuilder
{
    public class NodeVisitorHandler : INodeVisitorHandler
    {
        ...
        public async Task<NodeResponse> MatchLocationAsync(ITurnContext context, ImageHuntState state)
        {
            ...
            try
            {
                if (distance <= rangeDistance)
                {
                    ...
                    foreach (var nextActivity in nextActivities) await context.SendActivityAsync(nextActivity);
        ...
        public IList<Activity> ActivitiesFromNode(NodeResponse node)
        {
            ...
            {
                case NodeResponse.ObjectNodeType:
                    activities.Add(new Activity(text: string.Format(_localizer["NEXT_NODE_LOCATION"], node.Name),
                        type: ActivityTypes.Message));
                    activities.Add(new Activity(ImageHuntActivityTypes.Location)
                    {
                        Attachments = new List<Attachment>
                        {
                            new Attachment(
                                ImageHuntActivityTypes.Location,
                                content: new GeoCoordinates(
                                    latitude: node.Latitude,
                                    longitude: node.Longitude))
                        }
                    });
                    activities.Add(new Activity(text: string.Format(_localizer["DO_ACTION_REQUEST"], node.Action),
                        type: ActivityTypes.Message));
                ...
                case NodeResponse.WaypointNodeType:
                    ...
                    break;
                case NodeResponse.HiddenNodeType:
                    activities.Add(new Activity(text: string.Format(_localizer["HIDDEN_NODE"], node.Name),
                        type: ActivityTypes.Message));
                ...
                case NodeResponse.TimerNodeType:
                    activities.Add(new Activity(text: string.Format(_localizer["TIMER_NODE"], node.Delay),
                        type: ActivityTypes.Message));
                    activities.Add(new Activity(ImageHuntActivityTypes.Wait,
        ...
        public async Task MatchLocationDialogAsync(NodeResponse node,
