...
namespace ImageHuntBotBuilder.Commands
{
    ...
    public class InitCommand : AbstractCommand, IInitCommand
    {
        ...
        public InitCommand(ILogger<IInitCommand> logger, IGameWebService gameWebService,
            ITeamWebService teamWebService, INodeWebService nodeWebService, IStringLocalizer<InitCommand> localizer)
        ...
        protected override async Task InternalExecuteAsync(ITurnContext turnContext, ImageHuntState state)
        {
            ...
            if (regEx.IsMatch(text))
            {
                ...
                Logger.LogInformation($"Init group for GameId={state.GameId} TeamId={state.TeamId}");
                ...
                if (state.Game == null || state.Team == null)
                {
                    ...
                    var unableToFindGame =
                        string.Format(Localizer["UNABLE_FIND_GAME"], state.GameId ?? 0, state.TeamId ?? 0);
                ...
                var confirmMessage =
                    string.Format(
                        ...
                        state.Team.Name, state.Game.Name,
                        state.Game.StartDate.ToString(new CultureInfo(state.Team.CultureInfo)));
